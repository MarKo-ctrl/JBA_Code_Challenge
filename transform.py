...
def extract_data(lines):
    ...
    for i in range(len(lines)):
		if re.search(r'Grid-\w+=\s+[0-9]+,\s*[0-9]+', lines[i]):
			for k in range(120):
				data_dict['Xref'].append(re.split(r'Grid-\w+=\s+', 			lines[i])[1].rstrip().split(',')[0])
				data_dict['Yref'].append(re.split(r'Grid-\w+=\s+', lines[i])[1].rstrip().split(',')[1].strip())
            else:
...
def datelist(dates):
    ...
    total_dates = lambda dt: dt.month + 12 * dt.year
    dlist = []
